...
func TestUpgradeHint(t *testing.T) {
	...
	// Only amd64 publishes packages: other architectures build from source.
	if got := UpgradeHint(InstallDeb, "v2.1.1", "386"); got != "git pull && make build" {
		t.Errorf("deb 386 hint: %q", got)
	}
	if got := UpgradeHint(InstallRPM, "v2.1.1", "arm64"); got != "git pull && make build" {
		t.Errorf("rpm arm64 hint: %q", got)
	}
	if got := UpgradeHint(InstallDeb, "v2.1.1", "mips"); got != "git pull && make build" {
		t.Errorf("unknown arch must fall back to source build, got %q", got)
	}
...
func TestTarballName(t *testing.T) {
	...
	// Only amd64 publishes tarballs: other architectures build from source.
	if _, ok := TarballName("v2.1.1", "arm"); ok {
		t.Error("non-amd64 tarballs are not published")
	}
